...
def add_to_bag(request, item_id):
    try:
        ...
        for key, value in request.POST.items():
            if key not in [
                'csrfmiddlewaretoken', 'quantity', 'redirect_url'
            ] and value:
        ...
        if product.has_variation and variation_key:
            ...
            if str(item_id) in bag:
                if variation_key in bag[str(item_id)]['items_by_variation']:
                    bag[str(item_id)]['items_by_variation'][
                        variation_key
                    ]['quantity'] += quantity
                else:
                    bag[str(item_id)]['items_by_variation'][
            ...
            # Variation message
            variation_text = ', '.join(
                [f"{k}: {v}" for k, v in variations.items()]
            )
            messages.success(
                request,
                f"{product.product_name} ({variation_text}) added to your bag!"
            )
            ...
            if str(item_id) in bag:
                bag[str(item_id)]['quantity'] = (
                    bag[str(item_id)].get('quantity', 0) + quantity
                )
            else:
                bag[str(item_id)] = {'quantity': quantity}
            messages.success(
                request, f"{product.product_name} added to your bag!"
            )

        else:
            messages.error(
                request,
                f"Please select all required variations for "
...
def increment_bag_item(request, item_id):
    try:
        ...
        if product.has_variation:
            ...
            elif not variation_key or \
                    variation_key not in item_data['items_by_variation']:
                messages.error(
                    request,
                    f"Variation not found in your bag. Available keys: "
                    f"{list(item_data.get('items_by_variation', {}).keys())}"
                )
            else:
                current_qty = item_data[
                    'items_by_variation'
                ][variation_key]['quantity']
                if current_qty < 99:
                    item_data['items_by_variation'][
                        variation_key
                    ]['quantity'] += 1
                    messages.success(request, "Quantity increased.")
                else:
                    messages.error(
                        request, "Maximum quantity reached for this item."
                    )
        ...
        if str(item_id) in bag:
            ...
            if 'items_by_variation' in item_data and variation_key:
                if variation_key in item_data['items_by_variation']:
                    ...
                    messages.error(
                        request, "Variation not found in your bag."
                    )
