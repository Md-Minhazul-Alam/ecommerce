...
class WebsiteSetting(models.Model):
    ...
    website_thumbnail = models.ImageField(
        upload_to='website/', null=True, blank=True
    )
    ...
    website_office_hour = models.TextField(
        default='Mon-Fri : 10:00 AM - 5:00 PM', null=True, blank=True
    )
...
class QuickLink(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        slug = base_slug
        num = 1
        while QuickLink.objects.filter(slug=slug).exclude(
            pk=self.pk
        ).exists():
...
class LegalLink(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        slug = base_slug
        num = 1
        while LegalLink.objects.filter(slug=slug).exclude(
            pk=self.pk
        ).exists():
            slug = f"{base_slug}-{num}"
            num += 1
