...
class Tag(models.Model):
    ...
    def save(self, *args, **kwargs):
        base_slug = slugify(
            self.tag if not self.tag_slug else self.tag_slug
        )
        slug = base_slug
        num = 1
...
class Brand(models.Model):
    ...
    def save(self, *args, **kwargs):
        base_slug = slugify(
            self.brand if not self.brand_slug else self.brand_slug
        )
        slug = base_slug
        num = 1
...
class Category(models.Model):
    ...
    def save(self, *args, **kwargs):
        base_slug = slugify(
            self.category
            if not self.category_slug else self.category_slug
        )
        slug = base_slug
        count = 1
...
class Variation(models.Model):
    ...
    variation_slug = models.SlugField(
...
class Product(models.Model):
    ...
    def save(self, *args, **kwargs):
        base_slug = slugify(
            self.product_name
            if not self.product_slug else self.product_slug
        )
        slug = base_slug
        num = 1
...
class Review(models.Model):
    ...
    def __str__(self):
        return (
            ...
            f"{self.product.product_name} ({self.rating}★)"
        )